...
## MSX DDとは
- MSX-Cで作りかけたまま、4年以上寝かせているファイラです。**未完成品です。**
- スマホでも使いやすいようにと思い、SCREEN0:WIDTH40の画面モードで作っています。
- 狭い画面を有効活用するため、2画面をタブ形式で表示しています。
- 解像度を要求しないので、MSX0 Stack上（でDOS2が動かなければMSX0 Pro上）で活躍できるかもしれません。
...
- MAKEBAT.BAT
MSX-Cでこれらのソースファイルをコンパイルするための参考
※最後あたりでリンカL80のコマンドライン文字列制限にひっかからないようにするため、すべての.relファイルを2文字の短いファイル名にコピーしなおすという訳の分からないことをしています。
- MAKEFILE
自作MAKEのためのMAKEFILEですが、参考までに添付します。（MAKEBAT.BATはこのMAKEFILEから自作MAKEが生成しています）
- ディスクイメージ
...
同じ拡張子を、小文字と大文字で書くことができます。小文字は単にRETURNキーを押した場合、大文字はSHIFTキーを押しながらRETURNキーを押した場合に実行されます。
展開されるメタ文字は以下の通りです。
- $C カーソル位置のファイル名
- $D 別のタブのカーソル位置のファイル名
